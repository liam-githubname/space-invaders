...
void AssetManager::createScoreText(GraphicsModule &graphics, GameState &game_state) {
  ...
  SDL_SetTextureScaleMode(game_state.score_board_texture, SDL_SCALEMODE_PIXELART);
...
void AssetManager::createScoreNumber(GraphicsModule &graphics, GameState &game_state) {
  ...
  game_state.score_texture = SDL_CreateTextureFromSurface(graphics.getRenderer(), surface);
...
void AssetManager::createLivesCounter(GraphicsModule &graphics, GameState &game_state) {
  ...
  SDL_Texture *lives_texture =
    SDL_CreateTexture(graphics.getRenderer(), SDL_PIXELFORMAT_BGRA8888, SDL_TEXTUREACCESS_TARGET, width, height);
  ...
  for (int i = 0; i < game_state.number_of_lives; i++) {
    ...
    SDL_RenderTexture(graphics.getRenderer(), game_state.textures.sprite_sheet.get(), &player_sprite.frame1, &dstRect);
  }
...
void AssetManager::createMenuTextures(GraphicsModule &graphics, GameState &game_state) {
  ...
  SDL_SetTextureScaleMode(game_state.menu_title_texture, SDL_SCALEMODE_PIXELART);
  SDL_DestroySurface(title_surface);

  SDL_Surface *prompt_surface =
    TTF_RenderText_Blended(font_, "PRESS SPACE TO START", 0, SDL_Color{.r = 255, .g = 255, .b = 255, .a = 255});
  game_state.menu_prompt_texture = SDL_CreateTextureFromSurface(graphics.getRenderer(), prompt_surface);
  SDL_SetTextureScaleMode(game_state.menu_prompt_texture, SDL_SCALEMODE_PIXELART);
...
void AssetManager::createGameOverTextures(GraphicsModule &graphics, GameState &game_state) {
  ...
  SDL_SetTextureScaleMode(game_state.game_over_prompt_texture, SDL_SCALEMODE_PIXELART);
  SDL_DestroySurface(prompt_surface);
}
